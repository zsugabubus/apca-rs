...
impl<T: Float> LightnessContrast<T> {
    ...
    /// Returns whether `self` represents light text on dark background.
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// Returns whether `self` represents dark text on light background.
    ///
    /// # Examples
    ///
    /// ```
...
///
/// Implements <https://github.com/Myndex/SAPC-APCA/blob/master/documentation/APCA-W3-LaTeX.md>.
///
/// # Examples
///
/// ```
...
/// ```
#[inline]
pub fn lightness<T: TextOrBackgroundLightness>(color: T) -> Lightness {
    color.lightness()
}
...
mod tests {
    ...
    fn lightness_contrast() {
        ...
        for (text, background, expected) in vec![
            ...
            let actual =
                super::lightness_contrast(Text(to_color(text)), Background(to_color(background)))
                    .signed_value();
